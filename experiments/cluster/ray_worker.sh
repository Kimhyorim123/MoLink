...
# Start Ray worker on this node.
# Run this on node4/node6 and point to node5 head.

usage() {
  cat <<'EOF'
Usage: ray_worker.sh --node-ip IP --head IP:PORT
...
echo "[ray] starting worker: node-ip=$NODE_IP head=$HEAD"
"$RAY_BIN" start --address="$HEAD" --node-ip-address="$NODE_IP"
